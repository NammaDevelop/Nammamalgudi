...
export default function Navbar() {
  ...
  const active =
    TABS.find((t) => t.href === pathname)?.id ?? (pathname === '/' ? 'home' : '');
          ...
          <nav className="px-4 py-3 flex flex-col gap-1">
            ...
            <div className="flex gap-2 pt-3 pb-1">
              <Link
                href="/menu"
                onClick={() => setOpen(false)}
                className="flex-1 px-4 py-2.5 rounded-full bg-[#E87B33] hover:bg-[#C25E1F] text-white text-sm font-medium text-center"
              >
                Order Online
              </Link>
              <a
